...
sir_1_demography = function(beta, gamma, I0, R0, times, N, lambda, mu) {
  sir_equations = function(time, variables, parameters) {
    with(as.list(c(variables, parameters)), {
...
logli_sir_1_demography = function(beta, gamma, N, dat, lambda, mu) {
  I0 = dat$I[1]
  R0 = dat$R[1]
  times = dat[["time"]]
  beta = exp(beta)
  gamma = exp(gamma)
  predictions = sir_1_demography(beta = beta, gamma = gamma, I0 = I0, R0 = R0, 
