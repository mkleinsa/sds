...
#' @export 
plot.seirMod = function(fit, CI = FALSE, forecast = 0, mse_interval = TRUE, mse = NULL) {
    ...
    if(CI == TRUE) {
        ...
        resamp_stretch = rnorm(250, mean = 1, sd = model_error)
        resamp_jitter = rnorm(250, mean = 1, sd = model_error)
        ...
        p = p + 
            geom_line(data = data.frame(obs = c(data[["I"]], rep(0, forecast)), pred = best_predictions, time = plot_times), aes(x = time, y = pred), color = "#ff0505", linewidth = 1.4)
